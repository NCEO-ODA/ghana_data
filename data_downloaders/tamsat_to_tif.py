...
geoT = [-17.875, 0.25, 0, 37.375, 0., -0.25]
...
for year in range(2000, 2021):
    cmd = ["cdo", "cat", f"/gws/nopw/j04/odanceo/public/soil_moisture/nc/tamsat_nceo_da_wb_v1p0p0_{year}????.nc",
    ...
    tsteps = [x.strftime("%Y-%m-%d") for x in pd.to_datetime(ds.coords['time'].values)]
    doys = [int(x.strftime("%j")) for x in pd.to_datetime(ds.coords['time'].values)]


    for variable in ['precip', 'esoil_gb', 'ecan_gb', 'smc_avail_top', 'runoff']:
        ...
        for band, (tstep, doy) in enumerate(zip(tsteps, doys)):
            ...
            this_band.SetMetadata({'DoY':"%03d"%doy,
                                                    'Date':tstep})
