...
def mosaic_dates_wgs84_country(year, doy, folder="data", 
    ...
    """
    folder = Path(folder)
    files = [f'HDF4_EOS:EOS_GRID:"{f.as_posix():s}":MOD_Grid_MOD15A2H:{layer:s}'
         for f in folder.glob(f"{product:s}.A{year:04d}{doy:03d}.*hdf")]
    g = gdal.Warp((folder/f"{layer:s}.A{year:04d}{doy:03d}.tif").as_posix(), 
                 files, format=fmt, dstSRS=srs,
                 xRes=500, yRes=500,
...
year = int(sys.argv[1])
folder = Path("/gws/nopw/j04/odanceo/public/MCD15")
for layer in ["Lai_500m", "Fpar_500m", "FparLai_QC"]:
    fnames = []
    for doy in range(1, 367, 8):
        fname = mosaic_dates_wgs84_country(year, doy, "/gws/nopw/j04/odanceo/public/MCD15/hdfs",
                    product="MCD15A2H", layer=layer, fmt="GTiff")
        fnames.append(fname)
    ...
    for band in range(1, dst_ds.RasterCount + 1):
        ...
        doy += 8
    dst_ds = None
    g = gdal.Open((folder / f"{layer:s}_{year:d}.tif").as_posix(), gdal.GA_Update)
    ...
    shutil.move(folder / f"{layer:s}_{year:d}_temporary.tif", (folder / f"{layer:s}_{year:d}.tif").as_posix())
